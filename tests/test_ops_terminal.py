...
class OpsTerminalDocTests(unittest.TestCase):
    def test_covers_required_topics(self):
        ...
        self.assertIn("Agent Shell non-interactive", text)
        self.assertIn("no read/getpass", text)
        self.assertIn("Forbid `read`/`getpass` in agent scripts", text)
        self.assertIn("security -w last or it stores empty", text)
        self.assertIn("Secrets only in a **real Terminal**", text)
        self.assertIn("does not write Keychain", text)
        self.assertIn("does not run live secret entry", text)
        self.assertIn("Status/handoff: ETA until next Action required", text)
        ...
        self.assertIn("Warn before local-exec that may trigger macOS Allow sheets", text)
        self.assertIn("warn the operator before any local-exec", text)
        self.assertIn("macOS permission Allow sheets", text)
        self.assertIn("Documents/Desktop/Downloads", text)
        ...
        self.assertIn("No stacked ARs", text)
        self.assertIn("never stack Action required / card-like prompts in one turn", text)
        self.assertIn("exception only when the user explicitly asks for another AR", text)
        self.assertIn("active host-kept foreground job", text)
        self.assertIn("One machine, one command, loud banner still applies", text)
        self.assertIn("Do not stack open Action requireds", text)
        self.assertIn("CoS Desk default theater", text)
        self.assertIn("Merge ARs to Desk not 1:1", text)
        self.assertIn("default theater for factory Merge ARs", text)
        self.assertIn("status that needs user action", text)
        self.assertIn("Do not post Merge ARs to CoS private 1:1 unless the user asks for privacy", text)
        ...
        self.assertIn("card is in CoS 1:1 because rooms cannot show cards", text)
        self.assertIn("never call it private", text)
        self.assertIn("privacy from Jumpseat", text)
        self.assertIn("Developer Ready arrives on a private agent wake", text)
        self.assertIn("post Merge AR to Desk, not 1:1", text)
        self.assertIn("Watch proof: quote last sample or say not watching", text)
        self.assertIn("quote the last real sample line", text)
        self.assertIn("PT | job | alive/stalled/EXIT | n/N", text)
        ...
        self.assertIn("Factory docs batches", text)
        ...
        self.assertIn("Sent-from-machine / pasted proof", text)
        self.assertIn("loud SWITCH TO", text)
        self.assertIn("Agents cannot see which Terminal window is focused", text)
        self.assertIn("Continuous keepgoing", text)
        self.assertIn("immediate next AR after Done", text)
        ...
        self.assertIn("After user PASS on a check", text)
        self.assertIn("ack and next AR", text)
        self.assertIn("no re-ask", text)
        ...
        self.assertIn("Rem-aware SoR writer gate (look-ahead)", text)
        self.assertIn("refuse calendar SoR writers same cycle", text)
        self.assertIn("skip/rem-safe", text)
        self.assertIn("before the clock", text)
        self.assertIn("Classic MBP 8pm", text)
        self.assertIn("MAILROOM_DB=copy", text)
        ...
        self.assertIn("ATT-0 attachment lane constraints", text)
        self.assertIn("att0-constraints.md", text)
        self.assertIn("lane=auth", text)
        self.assertIn("Ready ≠ ATT implement", text)
        self.assertIn("never-purge attachment_*", text)
