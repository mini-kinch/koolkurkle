...
The paste is three lines. Timing is wall time at most 90 seconds (`PASS cold_request` / `FAIL cold_request`). A short body or `finish_reason` `length` does not flip that line. Cut-off is its own line, `PASS cold_truncated truncated=0` or `FAIL cold_truncated`. Short-answer is its own line, `PASS cold_content` / `FAIL cold_content`, and the bar is still content length greater than 300. `cold --idle` also prints `cold_probe`. The probe is not part of the length check. Before `OVERALL`, cold prints `FAILS` and `INVALIDS` (`none`, or the failed bar names). Overall PASS also needs an unchanged `ask_mail.py` hash.
