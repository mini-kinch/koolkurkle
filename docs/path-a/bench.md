...
Every soak request row, in the JSONL and on the progress line, records `finish_reason`, `content_len`, and `completion_tokens` (from `usage` when present, otherwise JSON null / progress `na`). The progress line also has `length=`. `TRUNCATED` is `finish_reason` `length`. `SHORT` is `content_len` <= 300. Both can be true. Neither is `ok`. No content length is `na`. The summary line next to timing is `PASS soak_length truncated=0 short=0` or `FAIL soak_length truncated=<n> short=<n>` with one `idx=` clause per flagged request. Before `OVERALL`, soak prints `FAILS` and `INVALIDS`. Each is `none` or the failed bar names (`soak`, `soak_length`, `soak_content`, `soak_mem_at`, `ask_mail`).
