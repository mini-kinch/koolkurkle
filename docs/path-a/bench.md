...
The paste is separate lines, read separately. Timing is wall time at most 90 seconds (`PASS cold_request` / `FAIL cold_request`). A short body, a missing answer fact, or `finish_reason` `length` does not flip that line. Cut-off is its own line, `PASS cold_truncated truncated=0` or `FAIL cold_truncated`.

When the paste has a sidecar `<fixture>.answer.json` (a `facts` list of strings, beside the `.txt`), the answer line replaces the short-answer bar. A reply passes when every required fact appears in the stripped assistant content. Matching is case-insensitive and collapses whitespace. An amount such as `18.00` also matches `18` and `$18.00` (a non-zero cent amount such as `12.40` stays `12.40` or `$12.40`). A date matches the ISO form in the key, for example `2026-02-14`. The lines are `PASS cold_answer matched=<k>/<n>` and `FAIL cold_answer missing=<facts> idx=<n>`. `content_len` is still printed on the progress line and stored on the JSONL row. It is not a pass/fail for a keyed paste, so a correct 80-character reply passes. The default paste `tests/fixtures/path_a/paste_4k_synthetic.txt` requires `syn-0007`, `2026-02-14`, `sender7@example.com`, and `18.00`, which is `PASS cold_answer matched=4/4`. The ten pastes in `tests/fixtures/path_a/paste_4k/` each have their own sidecar, derived from that file's DATA and QUESTION.

A paste with no sidecar keeps the short-answer bar: `PASS cold_content` / `FAIL cold_content`, content length greater than 300.

`cold --idle` also prints `cold_probe`. The probe is not part of the answer or length check. Before `OVERALL`, cold prints `FAILS` and `INVALIDS` (`none`, or the failed bar names, including `cold_answer` when that bar failed). Overall PASS also needs an unchanged `ask_mail.py` hash.
...
With `--continue-on-hang`, the hung row is kept and the next scheduled request still runs. The harness does not restart anything. The `PASS soak` / `FAIL soak` line is timing and hangs only: `failures=` on that line does not count a reply whose only miss is an answer fact, or, for a paste with no answer key, `content_len` <= 300 (`CONTENT_MIN`). HTTP 200 and `finish_reason` `stop` inside the timeout is a timing pass even when the body is short or missing a fact. A missing fact still fails the run (exit 1) through the answer line. It does not flip `PASS soak` or a hang's `next=pass`.

For a keyed paste the answer line replaces the short-answer bar and the SHORT label. `PASS soak_answer missing=0` means every scored reply contained every required fact. `FAIL soak_answer missing=<count> idx=<n> facts=<facts>` counts rows that missed any fact (a hang is not counted). `content_len` is still printed and is not a pass/fail. A correct 80-character reply passes. The default paste expects `PASS soak_answer missing=0` when the reply contains `syn-0007`, `2026-02-14`, `sender7@example.com`, and `18.00`.

A paste with no sidecar keeps `PASS soak_content short=0` or `FAIL soak_content short=<n> limit=content_len<=300 idx=<n> content_len=<len>`. Seen live on an unkeyed paste: 34 s, HTTP 200, `finish=stop`, `content_len=202` is a content FAIL and a timing pass. It is also SHORT on the length bar, not TRUNCATED. A keyed paste does not take SHORT from a short `content_len`. Overall PASS on `--continue-on-hang` means the timing line passed (zero timing failures, and every hang followed by a timing pass), the answer line passed when any paste was keyed, the content line passed when any paste was unkeyed, the length line passed, and the other bars passed. If `--watchdog-log FILE` is set, that file must also contain a `restart kickstart` line whose timestamp is within 5 minutes after the hang. The timestamp is the watchdog `log()` form `[YYYY-MM-DD HH:MM:SS]` (square brackets) or a bare `YYYY-MM-DD HH:MM:SS` at the start of a line. An `ok latency=` line in that window does not count. The file is only read, at the end of the run. A missing file, no `restart kickstart` line in that window, or a stamp outside that window fails the run (exit 1). The summary lists each hang with its local time and the recovery evidence (the next timing verdict, and the `restart kickstart` timestamp, `none`, or `missing`). If the log flag is omitted, recovery is only the next request. If nothing hung, a missing log is not a failure.
...
| `fail` | HTTP error, empty body, malformed JSON, bad response shape, `finish_reason` other than `stop`, a keyed reply missing a required fact, an unkeyed content length of 300 or less, warm/cold wall over the limit, or a non-timeout transport error. The run continues (except soak, which stops only on `hung`). |
...
`error` on a JSONL row is null for `pass`. Otherwise it is a short reason such as `timeout`, `empty body`, `malformed json`, `bad response shape`, `HTTP 500`, `finish_reason=length`, `content_len=300`, or `answer_missing=18.00`.
