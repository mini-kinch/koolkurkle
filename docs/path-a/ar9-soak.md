...
- `PASS soak_truncated truncated=0`, printed next to the timing `soak` line. `FAIL soak_truncated truncated=<n> idx=<n> finish=length content_len=<len> completion_tokens=<n>` means `finish_reason` is `length` (the reply was cut off). That cut-off still fails the timing `soak` line, because it is not a short-only or answer-only miss. The two lines name different bars. A correct reply that was cut off still fails this line.
- `PASS soak_length truncated=0 short=0`, the per-request length rollup beside timing. `TRUNCATED` means `finish_reason` is `length`. `SHORT` means `content_len` <= 300 (`CONTENT_MIN`) on a paste with no answer key. They are separate labels. A keyed paste does not take SHORT from a short body; `content_len` is informational. An unkeyed `finish=stop` reply with `completion_tokens=79` and `content_len=202` is SHORT, not TRUNCATED. One unkeyed reply can be both (`labels=TRUNCATED,SHORT`). A hang with no body is `length=na`, not SHORT.
- `PASS soak_answer missing=0`. This is the answer bar for pastes that have a sidecar key, and it replaces the short-answer bar for those pastes. It counts rows whose reply missed any required fact (`FAIL soak_answer missing=<count> idx=<n> facts=<facts>`). A hang is not counted. The default paste passes when the reply contains `syn-0007`, `2026-02-14`, `sender7@example.com`, and `18.00`. A correct 80-character reply passes. `content_len` is still printed and is not the bar. With `--continue-on-hang` an answer miss does not flip `PASS soak` or `next=pass`. `OVERALL` is still FAIL while this line is FAIL.
- `PASS soak_content short=0 limit=content_len<=300` remains the short-answer line for a paste with no answer key. It lists every unkeyed request with `content_len` <= 300, including one that is also cut off. Seen live on an unkeyed paste: 34 s, HTTP 200, `finish=stop`, `content_len=202` -> `FAIL soak_content` and `length=SHORT` (not TRUNCATED). With `--continue-on-hang` that short-only miss does not flip `PASS soak` or `next=pass`. A reply that is both cut off and short fails `soak_truncated`, `soak_length`, and `soak_content`. The default paste and the `paste_4k` set are keyed, so this run prints `soak_answer` rather than `soak_content`.
- Before `OVERALL`, two rollup lines name every bar that failed and every INVALID. A clean run prints `FAILS none` and `INVALIDS none`. A keyed run that missed facts and the mid-soak memory bar prints `FAILS soak_answer soak_mem_at`. An unkeyed run that missed content and memory prints `FAILS soak_length soak_content soak_mem_at`. Only the bars that failed are listed, in this order: `soak`, `soak_truncated`, `soak_length`, `soak_answer`, `soak_content`, `soak_mem_at`, `ask_mail`. Nothing that failed is visible only as `OVERALL FAIL`.
- `PASS soak_mem_at k=24` with `swap_used_mb` under 1024 and `ollama=down`.
- `PASS ask_mail` and `OVERALL PASS`.

Each request progress line and each JSONL request row records `finish` / `finish_reason`, `content_len`, and `completion_tokens`. `completion_tokens` comes from `usage` when that field is present, and is `na` on the progress line and JSON `null` when it is absent. The progress line ends with `length=ok`, `length=TRUNCATED`, `length=SHORT`, `length=TRUNCATED,SHORT`, or `length=na`. Cold and soak JSONL rows also store `content` (the stripped assistant text for a paste under `tests/fixtures/path_a/`, otherwise null), `answer_missing`, and `answer_key`.
